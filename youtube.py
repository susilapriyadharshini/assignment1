...
client=pymongo.MongoClient("mongodb+srv://susilapriyadharshini1:guviproject@cluster0.yrd0s.mongodb.net/?retryWrites=true&w=majority")
...
st.divider()   
channel_id = st.text_input(":green[Channel ID]")
...
display_table = st.radio(":green[_SELECT THE TABLE BELOW TO VIEW_]",(":green[Channels]",":green[Playlists]",":green[Videos]",":green[Comments]"))

if display_table == ":green[Channels]":
    display_channels_table()
elif display_table == ":green[Playlists]":
    display_playlists_table()
elif display_table ==":green[Videos]":
    display_videos_table()
elif display_table == ":green[Comments]":
    display_comments_table()

question = st.selectbox(':green[Curated Inquiries]',
